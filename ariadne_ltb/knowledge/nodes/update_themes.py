...
def update_themes(state: dict[str, Any], llm: KnowledgeLLM) -> dict[str, Any]:
    ...
    if not insights:
        return {"updated_themes": existing}
    response = llm.complete_json(update_themes_prompt(purpose, existing, insights), "SynthesisThemeDrafts")
