...
Ariadne 是一个面向 AI Builder 的 **Goal-driven Multi-Agent Build Team**。

它从一个 build goal 出发，协调 Research、Knowledge、Planner、Execution、Reviewer、Memory 等 Agent，把外部知识、项目上下文和历史记忆转化成可执行的 Build Tickets，再交给 Codex / Claude 等 coding agent 完成软件迭代。

## 固定对标

Ariadne 固定对标 Multica。

Multica 是 issue-driven agent team：

```text
Issue -> Assign to Agent -> Agent executes -> Progress / Review / Result
```

Ariadne 是 goal-driven agent team：

```text
Build Goal -> Research / Knowledge / Repo / Memory Agents -> Build Tickets -> Assign to Agents -> Codex / Claude executes -> Review -> Memory / Feishu / Next Tickets
```
...
4. 先固定能力面，再分批实现。
5. 优先补最能体现大厂 Agent 开发能力的部分：Goal-to-Ticket、多 Agent 协作、真实 Codex teammate、provider capability matrix、memory retrieval、review/eval。
