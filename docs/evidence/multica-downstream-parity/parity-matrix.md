...
```text
Ticket: M0TR-002
Assignment: assignment_699c4079b8ca
Agent: agent_4b050c358500 / Phase 3 Codex 1782402810345
Execution: execution_ad2a60e49e38
Outcome: BLOCKED_WITH_EVIDENCE
Reason: ARIADNE_ENABLE_EXTERNAL_EXECUTION must be 1
```

| Multica Surface | User Action | Visible Product Behavior | Underlying Objects | Multica Source Files | Ariadne Current State | Gap | Required Ariadne Object/API/UI | Browser Acceptance |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Issue assignment to agent | Assign a current version issue to a visible agent. | Issue detail shows assigned agent identity, backend, skill count, route, and handoff evidence. | `BuildTicket`, `TicketAssignment`, `AgentDefinition`, `RouteDecision`, `AgentHandoff`. | `views/issues/components/issue-detail.tsx`; `server/internal/handler/task_lifecycle.go`. | `M0TR-002` is assigned to `agent_4b050c358500`; issue detail evidence is captured in `issue-M0TR-002-after-runtime.json` and `browser-issue-M0TR-002-after-runtime.*`. | Real Codex execution remained gated, so the target repo was not modified. | Keep assignment as BuildTicket projection; route decisions must continue carrying `agent_id`, selected skills, runtime profile, and handoff path. | Passed with blocker evidence: `assignment_699c4079b8ca` is visible on `M0TR-002`, assigned to `Phase 3 Codex 1782402810345`, and blocked by explicit external execution gate. |
| Agent Detail / Tasks | Open the assigned agent and inspect its task list. | Agent detail tasks show assigned work, status, attempt, blocker, timestamps, and Inbox link for blocked work. | `TicketAssignment`, `InboxItem`, `BuildTicket`. | `activity-tab.tsx`; `tabs/task-failure.ts`; `task_lifecycle.go`. | Agent task projection captured in `agent-tasks-after-runtime.json`; browser DOM captured in `browser-agent-detail-tasks-after-runtime.*`. | No drag/drop or advanced queue filtering; this is intentionally out of v1 scope. | Preserve `AgentTaskProjection` as a read projection, not a separate task store. | Passed: `M0TR-002`, `assignment_699c4079b8ca`, and external execution blocker are visible in task/API evidence. |
| Agent Detail / Activity | Observe assignment lifecycle from agent detail. | Activity stream shows claim, planner, handoff, execution blocked, review, memory, board, and terminal blocker events. | `RuntimeEvent`, `TicketComment`, `Artifact`, `AgentRun`, `ExecutionResult`, `ReviewReport`. | `activity-tab.tsx`; `agent-live-peek-card.tsx`; `agent-activity-hover-content.tsx`. | Activity projection captured in `agent-activity-after-runtime.json`; browser DOM captured in `browser-agent-detail-activity-after-runtime.*`. | No WebSocket push; current v1 uses refresh/polling style evidence. | Continue deriving Activity from journal/assignment/run artifacts. | Passed: activity evidence includes `M0TR-002`, `assignment_699c4079b8ca`, `execution_ad2a60e49e38`, and `ARIADNE_ENABLE_EXTERNAL_EXECUTION` blocker. |
| Agent Detail / Skills / Instructions / Environment | Inspect agent capability tabs before/after run. | Skills, instructions, and environment key names are visible and are referenced by route/handoff evidence. | `AgentDefinition.skill_ids`, `AgentDefinition.instructions`, `AgentRuntimeProfile.environment_keys`, `RouteDecision`, `AgentHandoff`. | `skills-tab.tsx`; `instructions-tab.tsx`; `env-tab.tsx`; `runtime-config-tab.tsx`. | Browser DOM evidence captured in `browser-agent-detail-skills.*`, `browser-agent-detail-instructions.*`, and `browser-agent-detail-environment.*`; route/handoff evidence includes selected skill and environment key names. | Environment values are intentionally not displayed or persisted in product payloads. | Keep key-only environment metadata; never expose secret values in normal responses. | Passed: Workbench surfaces the real agent skill/instruction/env metadata; handoff evidence references the selected skill and environment key names. |
| Runtime page | Inspect runtime capability and assignment queue after run. | Runs page shows Codex/Claude capability, gated external execution state, assignment queue, blocked assignment, and disabled reasons. | `RuntimeCapability`, `TicketAssignment`, daemon status. | `runtimes-page.tsx`; `task_lifecycle.go`; `055_task_lease_and_retry.up.sql`. | Runtime evidence captured in `browser-runtime-claim.*` and `workbench-after-runtime.json`; product runtimes filter out internal fake/dry-run backends. | Real external execution requires explicit local gate; not enabled in this evidence run. | Keep Codex/Claude real backend scaffolds gated and visible as blocked when disabled. | Passed with blocker evidence: Runs page shows CodexBackend gated with `ARIADNE_ENABLE_EXTERNAL_EXECUTION is unset`, and `M0TR-002` blocked. |
| Inbox blocker / repair | Open Inbox after blocked execution. | Inbox lists blocker item tied to issue and agent, with safe actions; prior repair ticket evidence remains linked to agent/task. | `InboxItem`, `RepairAction`, `TicketAssignment`, `BuildTicket`. | `inbox-page.tsx`; `inbox-list-item.tsx`; `task-failure.ts`; `task_lifecycle.go`. | Inbox evidence captured in `inbox-after-runtime.json` and `browser-inbox-repair.*`; `M0TR-002` blocker is open and tied to `Phase 3 Codex 1782402810345`. | External gate blockers expose acknowledge/resolve actions; repair ticket creation remains reserved for repairable agent errors. | Continue classifying blockers before exposing repair/retry actions; do not make unsafe reruns automatic. | Passed: `M0TR-002` creates an Inbox item with `external_execution_blocked`; earlier ARI-077 repair action remains tied to same agent. |
| Task status lifecycle | Trigger runtime run-now on an assigned issue. | Assignment transitions from ready/claimable to blocked with timestamps and retry policy metadata; Issue/Agent/Inbox/Runs all show the same status. | `TicketAssignment`, `ExecutionResult`, `ReviewReport`, `RuntimeEvent`, `InboxItem`. | `task-status-pill.tsx`; `task_lifecycle.go`; `055_task_lease_and_retry.up.sql`. | `runtime-run-now-response-m0tr-002-clean.json` captures `did_work: true`, assignment blocked, `changed_files: []`, `test_exit_code: null`; execution artifact confirms empty `git_diff`. | Target project version did not advance because external execution was gated. | Treat gated execution as a first-class terminal blocker, not a fake success. | Passed with blocker evidence: canonical closure ledger is `BLOCKED_WITH_EVIDENCE`, no fake-codex/demo path used. |
